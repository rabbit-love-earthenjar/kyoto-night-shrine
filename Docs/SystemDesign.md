...
- `Stage_1_Route_Prototype` configures an isolated two-level route from the copied Stage 1 foundation. It starts at the upper-left, travels right across the upper route, drops through the far-right descent, and returns left on the lower route. Thick grass-tiled `BoxCollider2D` terrain represents the sketch's solid black outlines; only the vertical black marks are spike hazards. The right-side height differences prevent reverse climbing so progression remains downward there.
- The route reuses the Paper Doll and Ghost prefabs, existing grass/cloud/torii art, `BreakableBlock`, `HazardDamage`, `PickupItem`, `GameManager`, and Stage Clear flow without replacing `Stage_1_1`. Temporary clouds recover after their delay, while the special brown platform is safe on its first entry and drops immediately on the second separate entry until Retry resets it.
...
- Red Oni and bean-attack scripts are currently unconnected scaffolding. The intended design is a separate boss scene with a giant background-scale Red Oni; beans must be granted and used only inside that boss encounter.
