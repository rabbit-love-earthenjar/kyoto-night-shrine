...
- Restored the three-heart player HUD by keeping its `♥/♡` symbols on Unity's built-in symbol-capable font instead of routing them through the multilingual UI fallback.
- Softened runtime typography from heavy black outlines to Meiryo-first Japanese text with a very light shadow, and restyled the Hub shrine-repair dialog with a translucent dark-purple panel, warm-white body copy, brighter champagne-gold hierarchy, and Japanese close label.
- Lightened the farm presentation with warm wood-brown text, pale planting buttons, softer header backing, and reduced seed/action popup dimming so the field artwork remains visible and restful.
- Prevented repeated shared-style application from stacking multiple legacy UI shadows. Reusable TMP buttons now counter-scale their label against the selected plaque, retain a fixed text anchor, and use a much lighter outline; the prefab builder now leaves body/title text at normal weight until a button state deliberately emphasizes it.
- Rebuilt `Stage_1_Route_Prototype` as V33 with three clearly distributed one-heart recovery crates: two on the upper combat route and one at the lower-route landing.
- Kept Red Oni Final Rush dangerous after the white residual bar appears. Phase 4 now alternates Phase 1 lane attacks with Phase 2 platform-smash phrases, and retrying Phase 4 restores that mixed attack loop.
- Extended route and Boss Play Mode checks through the real gameplay path: every HUD heart must have a renderable glyph or Sprite, a recovery crate must restore one lost HP through `PickupItem`, and Final Rush must complete both a Phase 1 lane attack and a Phase 2 two-smash phrase before its speed-gate test can pass.
- Replaced remaining pure-black labels on pale Hub, cafe, Retry, and stage-clear buttons with the shared warm brown ink color; the cafe info close action now uses the Japanese `閉じる` label.

- Fixed a lethal-hit slowdown in `GhostHealth`: an enemy could destroy the component that owned the hit-stop coroutine while `Time.timeScale` was still `0.08`, leaving the rest of `Stage_1_2` permanently slowed. Hidden defeated enemies now survive only for the short real-time hit-stop cleanup window, and disabling the owner always releases its time-scale lock.
- Extended the Stage 1.2 combat Play Mode validation to assert that time scale returns to `1` after both Paper Doll and Ghost Lantern deaths.

- Added `NightShrineTextStyle` as the shared compatibility typography layer for runtime-generated UI. Menus, Hub/Farm/shop panels, cafe UI, boss prompts, result panels, and HUD values now prefer the same Japanese/Chinese-capable installed UI-font family without changing text anchors during selection.
- Kept reusable TMP menu typography visually stronger than body copy while reducing its outline weight; formal licensed font slots remain replaceable through `NightShrineUITheme`.

- Locked night-route node 2 during the first `HubArrival`; it becomes selectable only after the shrine cafe repair, while node 1 remains available for replay.
- Split the farm's combined seed picker/shop into two flows. Empty plots now show only warehouse-owned seeds and disable zero-stock choices; a dedicated rabbit-in-a-jar icon inside the farm opens a separate Faith-based seed shop using the existing `material_store_runtime/businessman.png` and `farm_icon` seed art.
- Audited all eight harvest-animation PNGs. Frames 3, 5, and 6 contain isolated soil fragments at the far-right source edge, so harvest playback now removes detached right-edge components at runtime while preserving intended sweat, motion lines, and victory sparkles. Source PNG and `.meta` files remain unchanged.
- Corrected first-Hub progression so the farm and rabbit ingredient-shop entrances no longer appear immediately after the first `Stage_1_2` clear. Both side activities, including their starter-stock initialization, now unlock only when the ruined shrine cafe is repaired; New Game also clears the static in-session repair state.
- Added functional StartScene `設定` and `クレジット` panels. Settings now persist BGM volume, sound-effect volume, title BGM preference (`ランダム` / `楽しい` / `悲しい`), 16:9 resolution, and window/fullscreen mode through the shared `GameSettings` service; starting a new game preserves these user preferences.
- Routed map-menu hover/confirm sounds, Faith Bean throws, and Red Oni impact sounds through the shared SFX-volume preference instead of bypassing the settings slider.
- Exposed the same title-music, resolution, and display controls in the global ESC settings panel. Selection animation enlarges button artwork while keeping text on a fixed anchor, preventing the recurring selected-label offset.
- Reworked the StartScene auxiliary panels as modal UI with a dark readability backing, stronger shrine-frame opacity, dedicated close/back actions, and factual provisional credits without inventing authorship or asset licenses.
- Removed the scene-owned `StartScene` EventSystem so the persistent `GlobalGameUiSystem` is the sole input-event owner from startup onward. Start-screen sprite loops, the foxfire selection cursor, menu selection transitions, and player visual/rotation presentation now write render state only when it actually changes, avoiding needless per-frame Canvas or SpriteRenderer invalidation on slower or thermally constrained machines. Added a Play Mode regression check for single-EventSystem ownership.
